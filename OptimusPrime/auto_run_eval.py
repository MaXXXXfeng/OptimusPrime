...
from OptimusPrime.data import process_text_data, create_text_dataloader
...
class AutoDeep(object):
    def __init__(self, task='text-cls', model_name=None, **kwargs):
        ...
        '''
        self.task = task
        MODEL_CONF.update_params(conf=kwargs)
    ...
    def process_data(self, train_path, test_path, val_path=None, **kwargs):
        ...
        if self.task in ['text-cls']:
            ...
            save = kwargs.get('save_vocab', False)
            frac = kwargs.get('split_frac', 0.7)
            tokenizer = kwargs.get('tokenizer', token_by_word)
            self.train_iter, self.val_iter, self.test_iter = process_text_data(train_path, test_path, val_path,
...
if __name__ == '__main__':
    # logger = Logger('test_log.txt').loggerImp()
    ...
    # model_path = '/Users/max/projects/OptimusPrime/data/text_cnn_model_1.pt'
    model_path = '/Users/max/projects/OptimusPrime/data/text_cnn_model_1'
    ...
    params = {'class_num': 10,
              ...
              'build_vocab': False,
              'print': False}
    ad = AutoDeep(model_name='LSTM', **params)
    ...
    # pre_param = {'vocab': vocab_path, 'label_map': label_map_rev}
